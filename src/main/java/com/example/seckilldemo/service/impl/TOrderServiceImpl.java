...
public class TOrderServiceImpl extends ServiceImpl<TOrderMapper, TOrder> implements ITOrderService {
    ...
    public TOrder secKill(TUser user, GoodsVo goodsVo) {
        ...
        //秒杀商品表库存减一，区别于redis库存减一，redis减库存是在stock.lua中进行的，
        seckillGoods.setStockCount(seckillGoods.getStockCount() - 1);
//        itSeckillGoodsService.updateById(seckillGoods);
        
        //判断库存大于0
        boolean Result = itSeckillGoodsService.update(new UpdateWrapper<TSeckillGoods>()
    ...
    //如果库存为0
        if (seckillGoods.getStockCount() < 1) {
            //判断是否还有库存，如果没库存，那就set成功了isStockEmpty这个值，等到TSeckillOrderServiceImpl轮询结果的时候，
        ...
        tSeckillOrder.setGoodsId(goodsVo.getId());//跟goods.getId是一样的
        itSeckillOrderService.save(tSeckillOrder);
        //订单生成成功之后，将秒杀订单信息存入redis,用户id+商品id作为key,后面可以防重复抢购
        redisTemplate.opsForValue().set("order:" + user.getId() + ":" + goodsVo.getId(), tSeckillOrder);
        return order;
    }
    @Transactional
    @Override
    public TOrder secKill2(TUser user, GoodsVo goodsVo) {//最原始的，不考虑用户登录，不考虑秒杀库存小于0
        //秒杀商品表减库存，先拿到秒杀商品
        TSeckillGoods seckillGoods = itSeckillGoodsService.getOne(new QueryWrapper<TSeckillGoods>().eq("goods_id", goodsVo.getId()));
        seckillGoods.setStockCount(seckillGoods.getStockCount() - 1);
        itSeckillGoodsService.updateById(seckillGoods);
